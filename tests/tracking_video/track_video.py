...
class SafePersonTracker(PersonTracker):
    ...
    def update(self, frame, detections, current_time=None):
        ...
        try:
            return super().update(frame, detections, current_time)
...
def process_video(video_path, output_path=None, skip_frames=0, confidence=0.5, headless=False):
    """
    ...
    try:
        while True:
            ...
            # Draw results
            try:
                result_frame = draw_results(frame.copy(), tracked_objects)
            except Exception as e:
...
if __name__ == '__main__':
    ...
    output_path = "out.mp4"  # Default to display only
    skip_frames = 0  # Process every frame
    confidence = 0.5  # Default confidence threshold
    headless = False  # Default to try using GUI
    ...
    if len(sys.argv) > 3 and sys.argv[3].lower() in ["--headless", "-h", "headless"]:
        headless = True
    ...
    # Process the video
    process_video(video_path, output_path, skip_frames, confidence, headless) 